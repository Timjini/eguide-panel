...
}" x-show="show" x-init="setTimeout(() => close(), {{ $timeout }})" x-transition:enter="transition ease-out duration-300"
    x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
    x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0"
    x-transition:leave-end="opacity-0 translate-y-2" class="fixed inset-x-0 top-4 z-50 mx-auto w-full max-w-md px-2">
    <div class="bg-white shadow-md rounded-lg pointer-events-auto ring-1 ring-black ring-opacity-5 overflow-hidden">
        <div class="p-3">
            <div class="flex items-start">
                <div class="flex-shrink-0">
                    <svg class="h-5 w-5 text-{{ $config['color'] }}-500" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
        ...
        <div class="bg-gray-100 h-0.5 w-full">
            <div x-data="{ width: 100 }" x-init="let interval = setInterval(() => {
